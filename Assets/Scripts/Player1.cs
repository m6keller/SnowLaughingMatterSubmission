...
public class Player1 : MonoBehaviour
{
    ...
    {
        // if up arrow
        if( IsGrounded( ) && Input.GetKeyDown( KeyCode.UpArrow) ) rigidbody.velocity = Vector2.up * jumpVelocity;
        HandleMovement( );
    }
    ...
    void HandleMovement( )
    {
        if( Input.GetKey( KeyCode.LeftArrow ) ) rigidbody.velocity = new Vector2( -moveSpeed, rigidbody.velocity.y );
        else if( Input.GetKey( KeyCode.RightArrow ) ) rigidbody.velocity = new Vector2( +moveSpeed, rigidbody.velocity.y );
